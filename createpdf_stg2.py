...
#EDITED ON 20230810 SUM OF DELER IN BOTH SALES PERSON AND DLER ANME   
    sqlx = "SELECT MEDIUM , COM_TYPE ||'-'||CASE WHEN BB_PACKAGE_NAME IS NULL AND SERO_ORDT_TYPE = 'CREATE' THEN 'CREATE' "\
"WHEN BB_PACKAGE_NAME IS NULL AND SERO_ORDT_TYPE = 'CREATE-UPGRD SAME NO' THEN 'UPGRADE' "\
"ELSE REPLACE(BB_PACKAGE_NAME,'LTE_','') END   XX , NVL(COM_STG2,0) , COUNT(EVENT_SOURCE) "\
"FROM SALES_"+TableName+"  WHERE (SALES_CHANNEL1 = '"+Dealer+"' OR SALES_PERSON11 = '"+Dealer+"') AND FINALSTATUS = 'OK' "\
"GROUP BY MEDIUM , COM_TYPE ||'-'||CASE WHEN BB_PACKAGE_NAME IS NULL AND SERO_ORDT_TYPE = 'CREATE' THEN 'CREATE' "\
"WHEN BB_PACKAGE_NAME IS NULL AND SERO_ORDT_TYPE = 'CREATE-UPGRD SAME NO' THEN 'UPGRADE' "\
"ELSE REPLACE(BB_PACKAGE_NAME,'LTE_','') END  ,NVL(COM_STG2,0) ORDER BY MEDIUM , "\
"COM_TYPE ||'-'||CASE WHEN BB_PACKAGE_NAME IS NULL AND SERO_ORDT_TYPE = 'CREATE' THEN 'CREATE' "\
...
"ELSE REPLACE(BB_PACKAGE_NAME,'LTE_','') END  "
    ...
    sql = "SELECT DEALER_NAME , NVL(SUM(SALES_COUNT_STG2),'0'), NVL(SUM(SALES_COUNT),'0') , NVL(SUM(TOT_STG2),'0') , NVL(SUM(PAYMRNT_STG1),'0') ,NVL(SUM(PAYMRNT_STG2),'0')  FROM SALES_COUNT_"+TableName+" WHERE DEALER_NAME = '"+Dealer+"' GROUP BY DEALER_NAME"
    #print(sql)
    cx.execute(sql)
    for ROW in cx:
        summaryTable = [
...
sql = "SELECT  '0', NVL(SUM(SALES_COUNT_STG2),'0'), NVL(SUM(SALES_COUNT),'0') , NVL(SUM(TOT_STG2),'0') , NVL(SUM(PAYMRNT_STG1),'0') , NVL(SUM(PAYMRNT_STG2),'0')  FROM SALES_COUNT_"+TableName+" WHERE DEALER_NAME IN ('Talentfort-Dealer','Talentfort-Franchise','Talentfort') "
    #print(sql)
cx.execute(sql)
for ROW in cx:
    summaryTable = [
